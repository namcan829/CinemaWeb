...
public class MovieServlet extends HttpServlet {
    ...
    MovieCategoryDAO cDao = new MovieCategoryDAO();
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Movies> movies;
        List<MovieCategory> listC = cDao.getAllCate();
        if (query != null && !query.trim().isEmpty()) {
            movies = mDao.searchMoviesByTitle(query);
        } else if (categoryId != null && !categoryId.isEmpty()) {
            movies = mDao.getMoviesByCategory(categoryId);
        } else {
            movies = mDao.getAllMovies();
        }
        request.setAttribute("categories", listC);
        request.setAttribute("listM", movies);
